...
class Report extends Model {
    ...
    public static function getReportData($profile) {
        ...
        $follows_count = Follow::where('twitter_profile_id', $profile->id)->whereDate('created_at', Carbon::today())->count();
        $unfollows_count = Unfollow::where('twitter_profile_id', $profile->id)->whereDate('created_at', Carbon::today())->count();

        // Friends
        $all_friends = Friend::where('twitter_profile_id', $profile->id)->get();
        $total_friends_count = $all_friends->count();
        $befriends_count = Befriend::where('twitter_profile_id', $profile->id)->whereDate('created_at', Carbon::today())->count();
        $unfriends_count = Unfriend::where('twitter_profile_id', $profile->id)->whereDate('created_at', Carbon::today())->count();
